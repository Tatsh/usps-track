...
If any changed files are under `usps_track/` or `tests/`, run the following
...
1. **click-auditor** - validate Click command consistency. **Only run if the project directly uses
   Click and files under `usps_track/commands/` changed.**
...
- **changelog** - update `CHANGELOG.md` only for **user-facing** changes (behaviour, CLI, public
  ...
  Files under `usps_track/`, `tests/`, or version changes in `pyproject.toml` are **candidates**
...
For example, if a commit contains `usps_track/commands/main.py`,
...
For Python files, strip the `usps_track/` prefix and replace `/` with `.` (like module imports).

- Python file `usps_track/media.py` → `media:`.
- Multiple files under `usps_track/commands/` → `commands:`.
- Single command file `usps_track/commands/admin.py` → `commands.admin:`.
