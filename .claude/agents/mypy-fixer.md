...
Fixes mypy type errors and eliminates `Any` usage in the usps-track project.
...
1. Fix all mypy errors reported by `uv run mypy usps_track`
...
Place new TypedDict classes in `usps_track/typing.py` if they are used across
modules, or locally if single-use. Add them to `__all__` in
`usps_track/typing.py` when shared.
...
1. Run `uv run mypy usps_track` and capture all errors.
...
4. Run `uv run mypy usps_track` again to verify fixes.
